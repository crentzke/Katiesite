...
this.$imageContainer.append('<span class="image-wrapper"><a class="advance-link" rel="history" ></a></span>').find('a').append(image).click(function(e){clickHandler(e,gallery,this);});}
...
stopIndex=this.data.length-1;var $thumbsUl=this.$thumbsContainer.find('ul.thumbs');$thumbsUl.find('li').each(function(i){var $li=$(this);if(i>=startIndex&&i<=stopIndex){$li.show();}else{$li.hide();}});$thumbsUl.removeClass('noscript');return this;},buildPager:function(pager){var gallery=this;var startIndex=this.currentPage*this.settings.numThumbs;if(this.currentPage>0){var prevPage=startIndex-this.settings.numThumbs;pager.append('<a rel="history" href="#'+this.data[prevPage].hash+'">'+this.settings.prevPageLinkText+'</a>');}
for(i=this.currentPage-3;i<=this.currentPage+3;i++){var pageNum=i+1;if(i==this.currentPage)
pager.append('<span class="current">'+pageNum+'</span>');else if(i>=0&&i<this.numPages){var imageIndex=i*this.settings.numThumbs;pager.append('<a rel="history" href="#'+this.data[imageIndex].hash+'">'+pageNum+'</a>');}}
var nextPage=startIndex+this.settings.numThumbs;if(nextPage<this.data.length){pager.append('<a rel="history" href="#'+this.data[nextPage].hash+'">'+this.settings.nextPageLinkText+'</a>');}
